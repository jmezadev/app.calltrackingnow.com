            ...
            var saleschart = function() {
                ...
                fetch('http://app.calltrackingnow.com/admin/controllers/calls/calls_controller.php')
                    ...
                    .then(function(myJson) {
                        // console.log(myJson.answered[0].calldate);

                        const totalAnswered = myJson.answered.length;
                        const arrayDates = [];
                        const arrayDatesIncluds = [];
                        var counter = 0;

                        for(var i = 0; i < totalAnswered; i++) {
                            var callDate = myJson.answered[i].calldate;
                            arrayDates.push(callDate.substring(0, 10));
                        }


                        var countDates = arrayDates.length;
                        console.log("------" + countDates);

                        for(var j = 0; j < countDates; j++) {
                            var date = arrayDates[j];

                            if (arrayDatesIncluds[date] === undefined) {
                                console.log("NO INCLUYE " + date);
                                arrayDatesIncluds[date] = 1;
                            } else {
                                console.log("INCLUYE " + date);
                                arrayDatesIncluds[date]++;
                            }
                        }

                        console.log(arrayDatesIncluds);

                        arrayDates.forEach(function (element) {
                ...
                if ($("#saleschart").length) {
                    ...
                    var d = [
                        [(new Date()).getTime() * 7, 1167],
                        [(new Date()).getTime() + 1, 342],
                        [(new Date()).getTime() + 2, 905],
                        [(new Date()).getTime() + 3, 906],
                        [(new Date()).getTime() + 4, 5000],
                        [(new Date()).getTime() + 5, 480],
                        [(new Date()).getTime() + 6, 454]
                    ];

                    var e = [
                        [(new Date()).getTime(), 234],
                        [(new Date()).getTime() + 1, 448],
                        [(new Date()).getTime() + 2, 710],
                        [(new Date()).getTime() + 3, 23],
                        [(new Date()).getTime() + 4, 5000],
                        [(new Date()).getTime() + 5, 112],
                        [(new Date()).getTime() + 6, 578]
                    ];

                    var f = [
                        [(new Date()).getTime(), 540],
                        [(new Date()).getTime() + 1, 88],
                        [(new Date()).getTime() + 2, 710],
                        [(new Date()).getTime() + 3, 673],
                        [(new Date()).getTime() + 4, 741],
                        [(new Date()).getTime() + 5, 66],
                        [(new Date()).getTime() + 6, 1086]
                    ];

                    var g = [
                        [(new Date()).getTime(), 257],
                        [(new Date()).getTime() + 1, 78],
                        [(new Date()).getTime() + 2, 710],
                        [(new Date()).getTime() + 3, 200],
                        [(new Date()).getTime() + 4, 540],
                        [(new Date()).getTime() + 5, 907],
                        [(new Date()).getTime() + 6, 756]
                    ];
